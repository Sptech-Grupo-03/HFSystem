...
senha char(10),
fkFazenda int,
constraint fkUsuarioEmpresa foreign key (fkFazenda)
	references fazenda(idFazenda)
);
...
insert into usuario (email,senha) values ('teste@gmail.com','#Teste123');
select * from fazenda;
select * from usuario;
desc reservatorio;
select * from reservatorio;

update reservatorio set nome = "Reservatório 1 Norte"
