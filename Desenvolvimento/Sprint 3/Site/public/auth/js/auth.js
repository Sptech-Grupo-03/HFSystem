...
document.getElementById("input_confirmacao_senha").addEventListener("keydown", function (event) {
    // Verifica se a tecla pressionada é o "Enter"
    if (event.key === "Enter") {
      // Impede o comportamento padrão do "Enter"
      event.preventDefault();
...
document.getElementById("formulario_cadastro").addEventListener("submit", function (event) {
    ...
    if (
      ...
      if (userNameValido && emailValido && validarSenha(input_senha_cadastro.value)) {
